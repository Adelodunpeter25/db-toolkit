...
export function QueryPlanModal({ isOpen, onClose, plan, query }) {
  ...
  const renderPlan = (planData) => {
    if (!planData) return <p className="text-gray-500">No plan data available</p>;
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Query Execution Plan">
      <div className="space-y-4">
        <div>
          ...
          <pre className="bg-gray-100 dark:bg-gray-800 p-3 rounded text-xs font-mono overflow-auto">
            {query}
          </pre>
        </div>
        <div>
